...
export default function AnswerCard({ uac, onClick, selectable = true, selected = false, pick, of }) {
    ...
    return (
        <div className="relative">
            <label
                className={selected ?
                    `card shadow border border-black bg-white outline outline-accent outline-4 select-none text-black text-md font-normal shrink-0 w-48 h-48 p-2 m-1 overflow-y-auto ${selectable ? "cursor-pointer" : ""}`
                    :
                    `card shadow border border-black bg-white text-black select-none text-md font-normal shrink-0 w-48 h-48 p-2 m-1 overflow-y-auto ${selectable ? "cursor-pointer" : ""}`
                }
